...
void RunSim() {
  double dt, probUpdateE, probUpdateI, uNet, spinOld, uExternalE, uExternalI;
  ...
  printf("prob update E = %f, prob update I = %f\n", probUpdateE, probUpdateI);
  ...
  for(i = 0; i < nSteps; i++) {
    ...
    if(UniformRand() < probUpdateE) {
      ...
      uNet = netInputVec[updateNeuronIdx] + uExternalE - THRESHOLD_E;
    }
    else {
...
int main(int argc, char *argv[]) {
  ...
  if(argc > 4) {
    trialNumber = atof(argv[4]); // parameter gamma
  }
